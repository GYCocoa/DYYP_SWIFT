...
- (NSString *)encryptByRsaWith:(NSString *)str keyType:(KeyType)keyType {
   // NSString *orstr = [str stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    NSString *orstr = [NSString stringWithCString:[str UTF8String] encoding:NSUnicodeStringEncoding];
    NSLog(@"%@",orstr);
    ...
    for (NSInteger i = 0; i < ceilf(str.length / 117.0); i ++) {
        ...
        NSString *ss = [CRSA bcd2Str:[self encryptByRsaToData:subStr withKeyType:(keyType)]];
